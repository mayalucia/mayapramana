...
MāyāPramāṇa lessons follow the [[https://github.com/mayalucia/mayalucia][MāyāLoom]] convention — org files
structured for both human and machine readers. The full manifesto is
in the parent project at =project/mayaloom.org=.
...
| =:level= | yes | Target: =undergrad=, =first-year-grad=, =researcher= |
...
2. Find the cadenza matching the student's question
3. Read =:level= and =:prereqs= to calibrate pitch
4. Read surrounding lesson context (at minimum the enclosing section)
5. Generate a tutorial that extends — not repeats — the lesson
