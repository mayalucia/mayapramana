...
This document defines how MāyāPramāṇa is organised, written, built, and versioned. It inherits conventions from MayaPortal and MayaDevGenI, adapted for a multilingual (Python, Haskell, C++) scientific project aimed at both human and machine intelligences.

* The Cardinal Rule

The =.org= files are the source of truth. Generated source files (=.hs=, =.hpp=, =.py=) are artifacts extracted via =org-babel-tangle=.
...
Never edit generated source files directly. Changes will be overwritten on next tangle, and the lesson narrative will desynchronise from the code.
...
Each lesson is cumulative: it imports and extends prior lessons' types and functions.
...
* The Physics
  Mathematical development, equations, physical intuition. LaTeX for equations, prose for meaning.

* Implementation: Python
  Interactive exploration. Runs in-session via org-babel. Plots, numerical experiments, building intuition.

* Implementation: Haskell
  Executable specification. Tangles to source tree. Types document the physics. Purity is guaranteed.

* Implementation: C++
  Deployment code. Header-only where possible. constexpr, concepts, dimensional analysis.
...
If they disagree, =plan.org= is authoritative. The plan articulates intent; the spec is its mechanical realisation.
...
Inherited from MayaPortal. All physics, signal processing, and estimation lives in the pure core. Hardware interaction lives in the shell.
...
The pure core is where the physics lives. The shell is as thin as possible. A different lab can swap their hardware interface without touching physics code.
...
2. *Cross-language validation*: Python, Haskell, C++ must agree on the same physics to within floating-point tolerance
3. *Property-based tests* (Haskell QuickCheck): "for all valid inputs, this invariant holds"
...
Fast re-entry point for any AI agent (Claude, GPT, Gemini, or other). Contains: project topology, active sessions, paused sessions, completed sessions with links.
...
Never paper over uncertainty with fluent prose. If you don't know, say so.
...
Scope: =physics=, =signal=, =estimation=, =control=, =lesson=, =haskell=, =cpp=, =python=
...
* MāyāLoom Conventions

MāyāPramāṇa lessons follow the [[https://github.com/mayalucia/mayalucia][MāyāLoom]] convention — org files structured for both human and machine readers. The full manifesto is in the parent project at =project/mayaloom.org=.

** Loom Manifest

Every =concept.org= carries a machine-readable header:

#+begin_example
,#+LOOM:     lesson
,#+DEFINES:  BlochState BlochParams bloch_derivative rk4_step simulate
,#+DEPENDS:  (none)
,#+EXPORTS:  bloch_precession.png
,#+CADENZAS: spin-half-qm bloch-sphere-geometry numerical-ode-methods
#+end_example

| Keyword | Purpose |
|---------+---------|
| =#+LOOM= | Document type: =lesson=, =plan=, =seed= |
| =#+DEFINES= | Concepts/functions this lesson introduces |
| =#+DEPENDS= | Other lessons or prerequisites required |
| =#+EXPORTS= | Artifacts produced (reference data, plots) |
| =#+CADENZAS= | Named expansion points, discoverable by agents |

** Cadenza Blocks

Expansion points where an agent generates a contextual tutorial:

#+begin_example
,#+BEGIN_CADENZA
:concept      spin-half-qm
:knows        classical-mechanics angular-momentum
:needs        quantum-spin-states bloch-sphere
:prereqs      classical-mechanics angular-momentum
:assumes      larmor-frequency gyromagnetic-ratio
:anti-targets quantum-field-theory relativistic-effects
:connects-to  bloch-sphere-geometry
[Prose prompt describing what to explain, for whom, from what
starting point, connecting to what notation in this lesson.]
,#+END_CADENZA
#+end_example

| Field | Required | Meaning |
|-------+----------+---------|
| =:concept= | yes | Slug identifying this cadenza |
...
| =:assumes= | no | What this lesson already taught — do not repeat |
| =:anti-targets= | no | Topics to explicitly not cover |
| =:connects-to= | no | Forward references to later sections/cadenzas |

** Agent Protocol for Cadenza Expansion

1. Read the =#+LOOM= manifest to orient
2. Find the cadenza matching the collaborator's question
3. Read =:knows= and =:needs= to identify the domain gap; read =:prereqs= to calibrate pitch
4. Read surrounding lesson context (at minimum the enclosing section)
5. Generate a tutorial that bridges the gap — extending, not repeating, the lesson
6. Use the same notation, variable names, and physical system
