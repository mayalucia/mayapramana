#+title: Lesson 00 — Chalkboard Illustrations
#+subtitle: Drawing tools for the Bloch equations lesson
#+author: MāyāPramāṇa
#+date: February 2026
#+LOOM: illustration-module
#+DEPENDS: mayapramana.chalkboard
#+property: header-args :mkdirp yes :eval never-export
#+property: header-args:python :session none :results silent :eval no-export

This file develops the chalkboard illustrations for Lesson 00 in
literate style.  The physics narrative lives in =concept-feynman.org=;
the drawing code lives here.

Run =org-babel-tangle= (=C-c C-v t=) to regenerate =illustrate.py=.

All functions follow the convention: call with default arguments to
reproduce the lesson figure; pass keyword arguments to customise.

* Preamble

#+name: illustrate-init
#+header: :tangle ./illustrate.py :comments both :padline yes
#+begin_src python
"""
...
The first board the student sees: experimental apparatus on the left,
free induction decay signal on the right.  Two panels, default 60/40
split (the schematic gets more room).

The function exposes ~T2_ms~ and ~f_visual~ so a learner can see how
the envelope and oscillation change.  The visual frequency is
compressed — 175 kHz would be invisible at this time scale.

#+name: illustrate-setup
#+header: :tangle ./illustrate.py :comments both :padline yes
#+begin_src python
...
A single atom's spin $\mathbf{M}$ precessing around $\mathbf{B}$.
The 3D cone is rendered via an oblique projection: the mapping

$$
(x, y, z) \mapsto (x - 0.175\,y,\; z + 0.105\,y)
$$

gives a "looking slightly from above and to the right" view.  The
back half of the orbit is dashed, the front half solid — the standard
crystallography convention for showing what's behind the sphere.

Customisable: ~theta_deg~ (cone half-angle, default 40°) and
~phi_deg~ (snapshot azimuth of $\mathbf{M}$, default 30°).

#+name: illustrate-precession-cone
#+header: :tangle ./illustrate.py :comments both :padline yes
#+begin_src python
...
* Relaxation: $T_2$ and $T_1$

Two panels showing the two faces of relaxation.

** Left: transverse decay ($T_2$)

Top-down view of the $xy$-plane.  The transverse magnetisation
$M_\perp$ spirals inward as atoms dephase.  The spiral is drawn as 20
segments with fading alpha and decreasing linewidth — the chalk
getting fainter as coherence fades.

Ghost M vectors at three intermediate times show the shrinking.
Six individual spins (yellow) fan out at $t = T_2/3$ to show what
"dephasing" means physically: each atom is still precessing, but at
slightly different rates, so the average shrinks.

** Right: longitudinal recovery ($T_1$)

$M_z(t)$ plotted against time in units of $T_1$.  Two curves:
recovery from $M_z = 0$ (saturation recovery) and from $M_z = -M_0$
(inversion recovery).  The inversion-recovery curve crosses zero —
this is the null point used in NMR to measure $T_1$.

#+name: illustrate-relaxation
#+header: :tangle ./illustrate.py :comments both :padline yes
#+begin_src python
...
Three equal panels, one per term in the Bloch equations.  Each shows
a Bloch sphere with the relevant physics:

1. *Precession* ($\gamma \mathbf{M} \times \mathbf{B}$) — rotation
   around $\mathbf{B}$, magnitude preserved.

2. *$T_2$ dephasing* ($-M_\perp / T_2$) — transverse component
   shrinks toward the $z$-axis, blue arrow shows the inward pull.

3. *$T_1$ recovery* ($-(M_z - M_0)/T_1$) — depleted $\mathbf{M}$
   grows back toward equilibrium $M_0$ (yellow dot).

** Bloch sphere drawing

Every panel needs the same sphere outline.  The helper
~draw_bloch_sphere~ draws the outer circle, equatorial ellipse
(oblique projection), meridian, and coordinate axes.  The helper
~plot_masked~ draws contiguous segments of a curve where a boolean
mask is true — used to dash the back half of the precession orbit.

#+name: illustrate-bloch-terms
#+header: :tangle ./illustrate.py :comments both :padline yes
#+begin_src python
...
The three-step sequence of an atomic magnetometer: pump, precess,
read.  Three equal panels.

Each panel shares the same cell geometry (~draw_cell~), field arrow
(~draw_B_field~), and Bloch sphere inset (~draw_bloch_inset~).  The
inset uses a simple side-view projection: $M_x$ maps to horizontal,
$M_z$ to vertical.

** Panel 1: Pump

Circularly polarised laser enters from the left.  Spins inside the
cell are nearly aligned (small random spread around $\hat{x}$).  The
Bloch inset shows $M_z = M_0$ — full polarisation.

** Panel 2: Precess

Pump off.  Spins are now at random azimuths (coherent precession, but
viewed as a snapshot the phases are random).  The Larmor frequency
$\omega_L = \gamma |\mathbf{B}|$ is labelled.

** Panel 3: Read

Probe laser enters from below.  Faraday rotation $\theta_F \propto
M_x$ converts precession into a polarisation rotation.  The
photodetector (PD) sees a damped oscillation — the free induction
decay signal.

#+name: illustrate-magnetometer
#+header: :tangle ./illustrate.py :comments both :padline yes
#+begin_src python
